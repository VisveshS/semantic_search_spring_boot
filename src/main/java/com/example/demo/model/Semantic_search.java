...
public class Semantic_search {
    ...
    public ArrayList<Struct<String, String, Float>> query(String query_string) throws IOException, InterruptedException {
        while (!constructed) {wait();}
        writer.println("query");
        writer.println(query_string);

        ArrayList<Struct<String, String, Float>> best_matches = new ArrayList<Struct<String, String, Float>>();
        // Print output
        ...
        try {
            while ((line = reader.readLine()) != null) {
                ...
                best_matches.add(new Struct<String, String, Float>(temp[0], temp[1], (float)Math.pow(score,1)));
                nline = nline - 1;
                if(nline == 0)
                    break;
            }
